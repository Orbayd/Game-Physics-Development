...
SmokeParticle::SmokeParticle(GameObjectName::Name name, GraphicsObject_Sprite *pSprite, GameSortBucket *pSort)
	:Particle(name, pSprite, pSort)
{

}

SmokeParticle *SmokeParticle::Create(GameObjectName::Name gObjName, ImageName::Name imageName, Rect rect, GameSortBucket *pSort)
{
	assert(pSort);
	...
	SmokeParticle *pParticle = new SmokeParticle(gObjName, pSprite, pSort);
	assert(pParticle);
